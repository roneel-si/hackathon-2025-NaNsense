...
function createEmbeddingFunction(): any {
	if (!process.env.OPENAI_API_KEY) {
		console.log(
			"⚠️  OPENAI_API_KEY not found, will use basic text storage",
		);
		return null;
	}
	...
	try {
		...
		console.log(
			"⚠️  Failed to create embedding function, falling back to basic storage",
		);
		return null;
	}
}
...
export async function getTable() {
	...
	try {
		...
		console.log("🔍 Count: inside getTable", count);
		console.log("🔍 Table: inside getTable", tbl);
		return tbl;
	} catch {
...
export async function searchTable(query: string, limit: number = 10) {
	...
	console.log("🔍 Searching table with query:", query);
	...
	try {
		...
		console.log(
			`✅ Semantic search successful, found ${results.length} results`,
		);
		...
		const results = await tbl
			.query()
			.filter(`emb_text LIKE '%${query.replace(/'/g, "''")}%'`)
			.limit(limit)
			.toArray();
