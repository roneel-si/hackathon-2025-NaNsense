...
export function createTriviaController({
	triviaService,
}: Dependencies): TriviaController {
	return {
		async generateSportsTrivia(
			request: FastifyRequest,
			reply: FastifyReply,
		): Promise<void> {
			try {
				...
				if (true || rag) {
					try {
						...
						const cleanedQuery =
							await triviaService.cleanAndValidateQuery(
								prompt.trim(),
							);

						// Search LanceDB for relevant data based on the cleaned prompt
						console.log(
							"Searching LanceDB for relevant data based on the cleaned prompt",
							cleanedQuery,
						);
						const searchResults = await searchTable(
							cleanedQuery,
							5,
						);
						...
						if (searchResults && searchResults.length > 0) {
							console.log(
								`Found ${searchResults.length} relevant results in database`,
							);

							// Create an enhanced prompt with context from the database using type-specific parsers
							const contextData =
								ContextParser.createContext(searchResults);
                ...
                `;
							...
							triviaData =
								await triviaService.generateEnhancedSportsTrivia(
									contextData,
									prompt.trim(),
								);
						} else {
							console.log(
								"No relevant data found in database, returning insufficient data response",
								prompt,
							);
							triviaData =
								await triviaService.generateSportsTrivia(
									prompt.trim(),
								);
						...
						console.error(
							"Error accessing vector database:",
							error,
						);
						// Fallback to standard trivia generation
						triviaData = await triviaService.generateSportsTrivia(
							prompt.trim(),
						);
					}
				} else {
