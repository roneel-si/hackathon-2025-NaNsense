...
export function createTriviaService(dependencies: Dependencies): TriviaService {
	...
	const apiKey =
		process.env.GEMINI_API_KEY || "AIzaSyBs2mo8LPzi_cCQVYLSlx4VHGzxzvPXwzM";
	...
	return {
        ...
        Generate 5 cricket focused trivia questions about: ${prompt}`;
				...
				if (!jsonMatch) {
					throw new Error(
						"Failed to generate valid JSON response from AI",
					);
				}
				...
				if (triviaData.data.length !== 5) {
					throw new Error(
						`AI did not generate exactly 5 questions. Got ${triviaData.data.length} questions.`,
					);
				}
				...
				for (let i = 0; i < triviaData.data.length; i++) {
					...
					if (question.Answer.length !== 1) {
						console.warn(
							...
							} answers. Forcing to single answer.`,
						);
				...
				for (let i = 0; i < triviaData.data.length; i++) {
					...
					if (!hasSportsKeywords) {
						console.warn(
							`Question ${i + 1} may not be sports-related: "${
								question.Question
							}"`,
						);
					}
				...
				return {
					data: [
						{
							Question:
								"Who was the last captain to win all cricket tournaments?",
							Options: [
								"MS Dhoni",
								"Virat Kohli",
								"Rohit Sharma",
						...
						},
						{
							...
							Options: ["France", "Croatia", "Belgium", "Brazil"],
							Answer: ["France"],
						},
						{
							Question:
		...
		async generateEnhancedSportsTrivia(
			enhancedContext: string,
			originalPrompt: string,
		): Promise<TriviaResponse> {
			try {
        ...
        Generate questions about: ${originalPrompt}`;
				...
				if (!jsonMatch) {
					throw new Error(
						"Failed to generate valid JSON response from enhanced AI",
					);
				}
				...
				if (!triviaData.data || !Array.isArray(triviaData.data)) {
					throw new Error(
						"Invalid response structure from enhanced AI",
					);
				}
				...
				if (triviaData.data.length !== 5) {
					throw new Error(
						`Enhanced AI did not generate exactly 5 questions. Got ${triviaData.data.length} questions.`,
					);
				}
        ...
        Return ONLY the cleaned query, nothing else.`;
				...
				console.log(
					`🧹 Query cleaned: "${rawQuery}" → "${cleanedQuery}"`,
				);
				return cleanedQuery;
			} catch (error) {
